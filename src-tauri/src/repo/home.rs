...
pub fn get_home_data(conn: &Connection, profile: i64, block_adult: bool) -> AppResult<HomeData> {
    let continue_watching = user_data::continue_watching_filtered(conn, profile, 20, block_adult)?;
    let favorites = user_data::list_favorites_filtered(conn, profile, None, Some(20), block_adult)?;
    let recent_channels = user_data::recent_channels_filtered(conn, profile, 15, block_adult)?;
    let latest_movies = latest(conn, profile, "movie", 20, block_adult)?;
    let latest_series = latest(conn, profile, "series", 20, block_adult)?;

    let mut live_categories =
        catalog::list_categories_filtered(conn, "live", Some(profile), block_adult)?;
...
fn latest(
    ...
    let adult_filter = if block_adult {
        format!(
            " AND {}",
            catalog::adult_exclusion_sql(if kind == "movie" { "m" } else { "se" }, kind)
        )
    } else {
        String::new()
    };
    ...
    let rows = stmt
        .query_map(params![profile, limit], move |r| {
            ...
            Ok(MediaCard {
                ...
                position_secs: None,
                duration_secs: None,
                series_id: if kind_owned == "series" {
                    Some(id)
                } else {
                    None
                },
