...
export function PosterCard({ card, onOpen }: { card: MediaCard; onOpen: (c: MediaCard) => void }) {
  ...
  return (
    <button
      ...
      title={card.name}
    >
      <div className="relative aspect-[2/3] w-full overflow-hidden rounded-2xl border border-line/60 bg-surface shadow-lg ring-0 transition-shadow duration-300 group-hover:shadow-2xl group-hover:ring-2 group-hover:ring-accent/60">
...
export function ChannelCard({
  ...
  return (
    <button
      ...
      title={card.name}
    >
      <div className="relative flex aspect-video w-full items-center justify-center overflow-hidden rounded-2xl border border-line/60 bg-surface-2 shadow-md transition-shadow duration-300 group-hover:shadow-2xl group-hover:ring-2 group-hover:ring-accent/60">
        <FavoriteButton card={card} />
...
export function ContinueCard({ card, onOpen }: { card: MediaCard; onOpen: (c: MediaCard) => void }) {
  ...
  return (
    <button
      ...
      title={card.name}
    >
      <div className="relative aspect-video w-full overflow-hidden rounded-2xl border border-line/60 bg-surface shadow-lg transition-shadow duration-300 group-hover:shadow-2xl group-hover:ring-2 group-hover:ring-accent/60">
        <FavoriteButton card={card} />
        <CardImage src={card.image} alt={card.name} className="h-full w-full" />
